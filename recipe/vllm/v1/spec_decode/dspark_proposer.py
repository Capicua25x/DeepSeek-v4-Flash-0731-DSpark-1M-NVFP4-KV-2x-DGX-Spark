...
class DSparkProposer(SpecDecodeBaseProposer):
    ...
    def propose(
        ...
        # Non-uniform batch guard (repo issue: concurrency>=2 worker death).
        # Mixed prefill/decode batches flatten to unequal per-request rows;
        # the uniform DSpark draft kernels cannot draft such a step. Skip
        # speculation for this step (plain decode) instead of raising and
        ...
        if batch_size > 0 and (
            ...
            # Return a full-shape dummy draft (token 0 x k); the verifier
            # rejects it naturally next step. Zero-width tensors break the
            # rejection-sampler shape contract downstream.
